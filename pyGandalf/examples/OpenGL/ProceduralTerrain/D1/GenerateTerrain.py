...
import pyGandalf.examples.OpenGL.ProceduralTerrain.D1.Terrain as Terrain
...
def export_texture(heightmap, filename, map_size=(256, 256)):
    image = Image.new('RGB', map_size, 0)
    draw = ImageDraw.ImageDraw(image)
    for x in range(map_size[0]):
        for y in range(map_size[1]):
            draw.point((x, y), (int(heightmap[x][y] * 255), int(heightmap[x][y] * 255), int(heightmap[x][y] * 255)))
    image.save(filename)
    print(filename, "saved")
    return

def main():
    noiseSettings = Noise.NoiseSettings(37, 7, 2, 0.5, 2, False, 2, False, Noise.typeOfNoise.Perlin)
    terrain = Terrain.Terrain(200, 62, 256, False, Noise.typeOfFallOff.Circle, 2.2, 0.4, 0.2)
    Terrain.Terrain.AddNoiseLayer(terrain, noiseSettings)
    terrain.CreateHeightMap()
    export_texture(terrain.heights, "height_map.png", (terrain.mapSize, terrain.mapSize))
